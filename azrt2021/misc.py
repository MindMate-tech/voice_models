...
def calc_performance_metrics(scr, lbl):
    ...
    prd = (scr > .5) * 1
    # metrics
    # Use labels parameter to ensure 2x2 confusion matrix even when only one class is present
    met['mat'] = confusion_matrix(y_true=lbl, y_pred=prd, labels=[0, 1])
    
    # Handle case where confusion matrix might be 1D (shouldn't happen with labels=[0,1], but be safe)
    if met['mat'].size == 4:
        TN, FP, FN, TP = met['mat'].ravel()
    elif met['mat'].size == 1:
        # Only one class present - set appropriate values
        if np.all(lbl == 0):
            # All true labels are 0
            if np.all(prd == 0):
                TN, FP, FN, TP = met['mat'][0, 0], 0, 0, 0
            else:
                TN, FP, FN, TP = 0, np.sum(prd), 0, 0
        else:
            # All true labels are 1
            if np.all(prd == 1):
                TN, FP, FN, TP = 0, 0, 0, met['mat'][0, 0]
            else:
                TN, FP, FN, TP = 0, 0, np.sum(1 - prd), 0
    else:
        # Fallback: try to reshape or handle gracefully
        mat_flat = met['mat'].ravel()
        if len(mat_flat) == 4:
            TN, FP, FN, TP = mat_flat
        elif len(mat_flat) == 2:
            # 2x1 or 1x2 matrix - need to infer
            if np.all(lbl == 0):
                TN, FP, FN, TP = mat_flat[0], mat_flat[1] if len(mat_flat) > 1 else 0, 0, 0
            else:
                TN, FP, FN, TP = 0, 0, mat_flat[0] if len(mat_flat) > 0 else 0, mat_flat[1] if len(mat_flat) > 1 else 0
        else:
            # Unexpected shape - set defaults
            TN, FP, FN, TP = 0, 0, 0, 0
    
    N = TN + TP + FN + FP
    if N == 0:
        # No samples - return NaN metrics
        met['acc'] = np.nan
        met['balanced_acc'] = np.nan
        met['sen'] = np.nan
        met['spc'] = np.nan
        met['prc'] = np.nan
        met['f1s'] = np.nan
    ...
    met['sen'] = sen
    met['spc'] = spc
    met['prc'] = TP / (TP + FP) if (TP + FP) > 0 else 0.0
    met['f1s'] = 2 * (met['prc'] * met['sen']) / (met['prc'] + met['sen']) if (met['prc'] + met['sen']) > 0 else 0.0
    met['wt_f1s'] = f1_score(lbl, prd, average='weighted', zero_division=0)
    met['mcc'] = (TP / N - S * P) / np.sqrt(P * S * (1-S) * (1-P)) if (P * S * (1-S) * (1-P)) > 0 else 0.0
    try:
