...
# Custom functions for CSV structure without transcript columns
def has_transcript_simple(df_raw):
    """
    Simple version that doesn't require duration_csv_out_list_len column.
    Returns empty dataframe (no test holdout) if column doesn't exist.
    """
    if 'duration_csv_out_list_len' in df_raw.columns:
        return df_raw.loc[df_raw["duration_csv_out_list_len"] != "0"]
    else:
        # Return empty dataframe - no test holdout
        return df_raw.iloc[0:0]

def get_row_data_simple(row, pid, lbl, fns, csv_path=None, **kwargs):
    """
    Simple version that doesn't require duration_csv_out_list column.
    Uses empty strings for transcripts if column doesn't exist.
    Resolves file paths relative to CSV file location.
    """
    segment_audio = kwargs.get('segment_audio')
    segment_audio_kw = kwargs.get('segment_audio_kw', {})
    row_data_list = []
    
    # Resolve paths relative to CSV file location
    if csv_path:
        csv_dir = os.path.dirname(os.path.abspath(csv_path))
    else:
        csv_dir = os.getcwd()
    
    # Handle transcript files - use empty if column doesn't exist
    if 'duration_csv_out_list' in row and pd.notna(row['duration_csv_out_list']):
        transcript_fns = str(row['duration_csv_out_list'])
        transcript_fns = transcript_fns.replace('\\', '/')
        transcript_fns = transcript_fns.strip('[]').replace('\'', '').split(', ')
        has_transcripts = transcript_fns != [""] and len(transcript_fns) > 0
    else:
        transcript_fns = [""] * len(fns)
    ...
    for idx, fn in enumerate(fns):
        ...
        if not os.path.isabs(fn):
            fn_resolved = os.path.normpath(os.path.join(csv_dir, fn))
        else:
            fn_resolved = fn
        
        transcript = ""
        if has_transcripts:
            transcript = transcript_fns[idx]
            if transcript and not os.path.isabs(transcript):
                transcript = os.path.normpath(os.path.join(csv_dir, transcript))
        
        # check if file exists
        assert os.path.isfile(fn_resolved), f"{fn_resolved} not found (original: {fn})"
        if transcript != "":
            assert os.path.isfile(transcript), transcript
        this_row_data = [pid, fn_resolved, lbl, transcript]
        if segment_audio is not None:
            this_row_data.extend(segment_audio(fn_resolved, **segment_audio_kw))
        else:
...
def main():
    ...
    task_csv_txt = args.get('task_csv_txt', '../csv_files/task_csvs.txt')
    task_id = args.get('task_id', 0)
    csv_info, ext = select_task(task_id, task_csv_txt)

    get_label = None
    if '_vs_ad' in ext:
        # Support both 'is_ad' and 'is_demented_at_recording' column names
        def get_label_ad(r):
            if 'is_ad' in r:
                return int(str(r['is_ad']) == '1')
            elif 'is_demented_at_recording' in r:
                return int(str(r['is_demented_at_recording']) == '1')
            else:
                raise KeyError("Neither 'is_ad' nor 'is_demented_at_recording' found in CSV")
        get_label = get_label_ad
    ...
    if torch.cuda.is_available():
        ...
        if isinstance(device, str):
            # If it's already a string like 'cuda:0', use it as is
    ...
    for seed in seed_list:
        ...
        for i in range(num_folds):
            for j in range(num_folds):
                ...
                # Use custom functions that work with our CSV structure
                get_all_trn_test_func_kw = {'get_test_ids': has_transcript_simple}
                kwargs['get_all_trn_test_kw'] = get_all_trn_test_func_kw
                kwargs['get_row_data'] = get_row_data_simple
                # Ensure csv_path is passed to get_row_data_simple
                if 'csv_path' not in get_row_data_kw:
                    get_row_data_kw['csv_path'] = csv_info
                
                if test_transcript_mri:
                    # Override with transcript+mri function if needed
